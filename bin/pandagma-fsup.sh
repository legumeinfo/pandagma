...
Usage:
  ...
  Required:
           ...
           -f FAM_DIR ... name of directory from a previous pandagma-fam run, containing one of the two subdirectories:
               either
                 (1) hmm_dir='DIR_NAME'
                     ... if realigning to HMMs, provide a directory of HMMs, specified with hmmdir in the config file, 
                     with hmm files named as gene family names (if realigning to those HMMs).
                     The directory 21_hmm generated by \"pandagma fam\" is suitable.
                     Note that this is for the realignment process rather than for hmmsearch. 
                     For hmmsearch, separately provide the hmmdb_path.
               or
                 (2) prot_fam_dir='DIR_NAME' 
                     ... for search_method='align_sample', a directory of protein sequences in multifasta files with 
                     gene-family filenames. 
                     ...
                     The multifasta files are used as targets for comparison by the query sequences in the fsup job.
...
Variables in pandagma config file (Set the config with the CONF environment variable)
      ...
      search_method - Method for placing sequences into pangenes. One of: align_sample hmmsearch [hmmsearch]
            hmm_dir - Subdirectory within specified -f FAM_DIR that contains family HMMs [21_hmm], if search_method=hmmsearch
       prot_fam_dir - Subdirectory within specified -f FAM_DIR that contains protein files placed into multifasta
                        gene family files, if search_method=align_sample. 
                      In the output directory (default out_pandagma/, it is 19_pan_aug_leftover_merged_prot/.
...
canonicalize_paths() {
  ...
  echo "  fam_dir: $fam_dir"
  echo "  search_method: $search_method"
  echo "  hmmdb_path: $hmmdb_path"
  ...
  if [[ "$search_method" == "align_sample" ]]; then
    if ! [[ -d "$fam_dir/$prot_fam_dir" ]]; then
      echo "For search_method \"align_sample\", please also specify and provide a fam_dir and prot_fam_dir" >&2
      exit 1
    fi
  fi

  if ! [[ "$search_method" == "hmmsearch" ]]; then
    if ! [[ -v "$hmmdb_path" ]]; then
      echo "For search_method \"hmmsearch\", please also specify hmmdb_path (the FULL PATH to hmmpress-d HMMs)" >&2
      exit 1
    fi
  fi

  if ! [[ "$fam_dir/$hmm_dir" ]]; then
    echo "For search_method \"hmmsearch\", please also specify a directory of HMMs, named as the respective"
    echo "gene family IDs, to use for realigning the gene families."
  else 
    export FAM_HMM_DIR="$PWD/$fam_dir/$hmm_dir"
    echo "  FAM_HMM_DIR: $FAM_HMM_DIR"
  fi
...
run_search_families() {
  ...
  if [ -d 33_fam_match ]; then rm -rf 33_fam_match; fi
  mkdir -p 33_fam_match 

  if [ "$search_method" == "align_sample" ]; then
    mkdir -p 33_mmseqs_tmp
    ...
    for filepath in 02_fasta_prot_sup/*; do 
      ...
      echo "  mmseqs easy-search $filepath 19_palmp.sample.faa 33_fam_match/$base.x.19_palmp.fam_ID.m8 \\"
      echo "    33_mmseqs_tmp --search-type ${SEQTYPE} --cov-mode 0 -c ${clust_cov} 1>/dev/null"
      mmseqs easy-search "$filepath" \
                         19_palmp.sample.faa \
                         33_fam_match/"$base".x.19_palmp.fam_ID.m8 \
                         33_mmseqs_tmp \
    ...
    for filepath in 33_fam_match/*.m8; do
      base=$(basename "$filepath")
      < "$filepath" perl -pe 's/^(\S+)\t([^_]+)__\S+/$1\t$2/' | 
        sort -k1,1 -k2,2 | calc_avg_bitscore_per_fam_hit.awk | 
        sort -k1,1 -k12nr,12nr | top_line.awk > 33_fam_match/"$base".top &
    ...
    for filepath in 02_fasta_prot_sup/*; do 
      base=$(basename "$filepath" ."$faa")
      # Set --cpu 2 for hmmsearch because it is IO bound. Rather, get parallelization by running the searches in parallel.
      echo "== Search $base.$faa against a HMMer database (compressed and indexed HMMs)"
      echo "    hmmsearch -E $evalue --cpu 2 --tblout 33_fam_match/$base.hmmsearch.tbl -o /dev/null ${hmmdb_path} ${filepath}"
      hmmsearch -E "$evalue" --cpu 2 --tblout 33_fam_match/"$base".hmmsearch.tbl -o /dev/null "${hmmdb_path}" "${filepath}" &
      # allow to execute up to $NPROC/2 in parallel
      if [[ $(jobs -r -p | wc -l) -ge $((NPROC/2)) ]]; then wait -n; fi
    done
    wait

    echo "== Parse tabular hmmsearch data, picking the top match per query"
    for filepath in 33_fam_match/*.hmmsearch.tbl; do
      ...
      awk -v OFS="\t" '$1!~/^#/ {print $1, $3, $1, $5, $6}' "$filepath" |
        sort -k1,1 -k5nr,5nr | top_line.awk |
          strip_spliceform.pl > 33_fam_match/"${base}"."${hmmdb_name}".gfa.tsv &
       # allow to execute up to $NPROC in parallel
       if [[ $(jobs -r -p | wc -l) -ge ${NPROC} ]]; then wait -n; fi
    done
    wait
  
    echo "== Place sequences into families based on top protein match to HMM, using the E-value threshold of $evalue."
    cat 33_fam_match/*.gfa.tsv |
      awk -v EVAL="${evalue}" '$4<=EVAL {print $2 "\t" $1}' |
      sort -k1,1 -k2,2 | hash_to_rows_by_1st_col.awk > 34_sup_vs_fam_consen.clust.tsv
  ...
  perl -lane 'for $i (1..scalar(@F)-1){print $F[0], "\t", $F[$i]}' 34_sup_vs_fam_consen.clust.tsv > 34_sup_vs_fam_consen.hsh.tsv
}
...
run_realign_and_trim() {
  ...
  for filepath in 35_sup_in_fams_prot/*; do 
    ...
    hmmalign --trim --outformat A2M --amino -o 42_hmmalign/"$file" "$FAM_HMM_DIR/$file" 35_sup_in_fams_prot/"$file" &
    if [[ $(jobs -r -p | wc -l) -ge $((NPROC/2)) ]]; then wait -n; fi
  done
  wait
...
pandagma_conf_params='consen_iden clust_cov TE_match_iden annot_str_regex min_align_count min_annots_in_align 
evalue search_method hmmdb_path hmm_dir' 
...
NPROC=$( command -v nproc > /dev/null && nproc )
