...
define HELP_DOC <<'EOS'
Place annotation sets (CDS and protein) into pangene or gene family sets, using mmseqs easy-search
...
Usage:
  ...
  Required:
           ...
           -f FAM_DIR ... name of directory containing
               (1) A directory of HMMs, specified with hmm_dir in the config file, named as gene family names (if realigning to those HMMs).
...
run_fam_consen() {
  echo
  echo; echo "== Generate a sampling of sequences from each family =="
  cd "${WORK_DIR}" || exit
...
run_search_families() {
  cd "${WORK_DIR}" || exit
  echo "Search provided annotation sets (protein) against family consensus sequences"
