...
my $usage = <<EOS;
  Usage: cat FILE.aligncoords | $scriptname -fasta_db <dir_with_directory.index> -match_table FILE.m8 -report_out FILE [options]
  
  Calculates Ks for genes in DAGchainer synteny gene pairs.
  
  Required:
    [on STDIN]      Input from an .aligncoords file, from DAGchainer
    -fasta_db       Path to directory with Bio::DB::Fasta Berkeleydb file (directory.index)
                    for CDS sequence FASTA files containing genes from FILE.aligncoords
...
Required if -align_method is "precalc":
    -match_table    BLAST -m8 format file generated by mmseqs easy-search with --format-output
...
if ($align_method =~ /clustalw/i){
  ...
  # Don't assign $align_engine. We'll index & use the provided blast -m8 (-outfmt 6) alignment file.
  $blast_report_index = Bio::Index::BlastTable->new(-filename => "$match_table.index", -write_flag => 1);
  $blast_report_index->id_parser(\&get_id);
  $blast_report_index->make_index($match_table);
}
else {
...
while (my $thisline = <>) {
  chomp $thisline;
  next unless $thisline; # sometimes an empty string, even if one doesn't appear in input .aligncoords;
                         # possible subtle child process interference with stdin?
  ...
  if ($thisline =~ /^#/) {
    ...
    if ($align_method =~ /precalc/i){ # Seqs in prot aligns may be truncated/partial. Adjust prot and nucl seqs.
      ...
      my ($fh, $query, $reverse, $subject);
      # get_report() doesn't handle m8 + extra alignment columns, so using
      # get_stream() and searching for specified subject sequence
      eval {
        $fh = $blast_report_index->get_stream("$idA");
        $query = $idA;
        $subject = $idB;
      };
      if ($@) { # assume blast m8 output order swapped w.r.t. dagchainer output
        $fh = $blast_report_index->get_stream("$idB");
        $reverse = 1;
        $query = $idB;
        $subject = $idA;
      }
      my @fields;
      while( <$fh> ) {
        /^\S+__(\S+)__\d+__\d+__[+-]\t\S+__(\S+)__\d+__\d+__[+-]/;
        last if $1 ne $query;
        if ($2 eq $subject) {
          @fields = split if ($2 eq $subject);
          last;
        }
      }
      ...
      my ($protB_start, $protB_end, $protB_aln) = ($fields[8], $fields[9], $fields[13]);

      if ($reverse) { # swap values
        ($protA_start, $protA_end, $protA_aln, $protB_start, $protB_end, $protB_aln) =
          ($protB_start, $protB_end, $protB_aln, $protA_start, $protA_end, $protA_aln);
      }
...
# index blast (mmseqs) m8 format file by query
sub get_id {
   /^\S+__(\S+)__\d+__\d+__[+-]\t/;
   return $1;
}
