...
my $usage = <<EOS;
  ...
  Required if --align_method is "precalc":
    -match_table        Name of fourteen-column tabular match table with alignment sequences in cols 13 and 14
                          This is BLAST -m8 format -m8, generated by mmseqs easy-search with --format-output
  ...
  Options:
    ...
    -am/--align_method  Method for generating protein alignments. Options: clustalw, mafft, precalc [precalc]
...
if ($align_method =~ /clustalw/i){
  ...
  # Don't assign $align_engine. We'll just use the provided alignment.
}
else {
  die "Alignment method is not recognized. Available methods: clustalw, mafft, precalc";
}
...
##################################################
# If align_method is "precalc", read the file of tabular hit-scores + alignments (-m8+alignments from mmseqs)
my %match_table_hsh;
my $MATCH_FH;
if (defined $match_table){
  open ($MATCH_FH, "<", $match_table) or die "Can't open in match_table: $match_table$!\n";
}
while (<$MATCH_FH>){
  chomp;
  my $line = $_;
  ...
  if (scalar(@fields) != 14){ 
    die "The match-file, $match_table, has an unexpected line. Expected 14 fields but found\n$line\n";
  }
  my ($qry, $sbj) = ($fields[0], $fields[1]);
  # If the sequence IDs are "compound", with form chr__ID__start__end__orient, then extract and use the central IDs
...
while (<$DAG>) {
  ...
  if ($line =~ /^#/) {
    ...
    else { # $align_method is clustalw or mafft. Don't adjust sequence boundaries.
    }
...
sub align_pair {
  ...
  my $aa_aln_obj;
  if ( $align_method =~ /clustalw/i || $align_method =~ /mafft/i ){
