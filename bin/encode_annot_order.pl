...
foreach my $row ( @sorted_table ) {
  my ($pepID, $gene, $ann, $chr, $start, $end, $orient) = @$row;
  #say join("\t", "AA: ", $pepID, $gene, $ann, $chr, $start, $end, $orient);
  unless ( $seen_chr{$chr} ){ $seen_chr{$chr}++ }
  if ($ann eq $prAnn && $chr == $prChr){
    $ord++;
    @elts_with_order = ( $pepID, $ann, $chr, $ord, $start, $end, $orient );
...
foreach my $row ( @pangene_table_ordered ) {
  my ( $pepID, $ann, $chr, $order, $start, $end, $orient ) = @$row;
  my $chr_str = sprintf("chr%02d", $chr);
  if ($orient =~ /\+/){ 
    $pepID .= "Q";  # Mneomic: P (for Plus / + ) is next to Q
  }
  elsif ($orient =~ /-/){
    $pepID .= "N";  # Mneomic: N is for Negative (-)
  }
  ...
  if (! $seen_chr{$chr_str}){
    ...
    if (! $seen_ann_chr{$ann_chr}){
      ...
      $encoded_order_seq{$chr_str}{$ann} = "$pepID";
    }
    else {
      $encoded_order_seq{$chr_str}{$ann} .= "$pepID";
    }
